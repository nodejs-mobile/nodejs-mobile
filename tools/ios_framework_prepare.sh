...
# iOS Simulator compilation disabled because of https://github.com/nodejs-mobile/nodejs-mobile/pull/9#issuecomment-1359289496
...
for output_file in "${outputs[@]}"; do
    if [ -n "$IOSSIM" ]; then
        lipo -create "$TARGET_LIBRARY_PATH/arm64/$output_file" "$TARGET_LIBRARY_PATH/x64/$output_file" -output "$TARGET_LIBRARY_PATH/$output_file"
    else
...
if [ -n "$IOSSIM" ]; then
    ...
    lipo -create $FRAMEWORK_TARGET_DIR/Release-iphoneos/NodeMobile.framework/NodeMobile -output $FRAMEWORK_TARGET_DIR/Release-universal/NodeMobile.framework/NodeMobile
fi
...
if [ -n "$IOSSIM" ]; then
    ...
    xcodebuild  -create-xcframework -framework $FRAMEWORK_TARGET_DIR/Release-iphoneos/NodeMobile.framework -output $FRAMEWORK_TARGET_DIR/NodeMobile.xcframework
fi
